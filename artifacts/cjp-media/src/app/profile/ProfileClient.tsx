...
export default function ProfileClient({ initialPosts, profile }: { initialPosts: any[], profile: any }) {
  ...
  const pName = localProfile?.name || 'CJP Media';
  const pHandle = localProfile?.handle || '@cjpmedia';
  const pBio = localProfile?.bio || 'We speak for the ignored, the unseen,\nand the unemployed youth.';
  const pLocation = localProfile?.location || 'New Delhi, India';
  const pUrl = localProfile?.url || 'cjpmedia.in';
  const pJoined = localProfile?.joined || 'Jan 2024';
  const pFollowers = localProfile?.followers || '127K';
  const pAvatarUrl = localProfile?.avatarUrl;
  const pCoverUrl = localProfile?.coverUrl || 'https://picsum.photos/seed/cjpbg2/1000/600';
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#0a0a0a]">
       ...
       <div className="sticky top-0 bg-[#0a0a0a]/95 backdrop-blur-md z-50 flex items-center justify-between px-4 h-14 border-b border-white/5">
        <button onClick={() => window.history.back()} className="w-10 h-10 flex items-center justify-center text-white hover:bg-white/10 rounded-full transition-colors -ml-2">
          <ArrowLeft className="w-6 h-6" />
        </button>
        <h1 className="text-white font-semibold text-[17px] flex items-center">{pName} <VerifiedBadge className="w-4 h-4 ml-1" /></h1>
         ...
         <div className="px-4 relative pb-5 border-b border-white/5">
            <div className="flex justify-between items-start">
               ...
               <div className="relative -mt-[52px] z-10 shrink-0">
                 <div className="w-[104px] h-[104px] rounded-full border-[4px] border-[#0a0a0a] bg-[#121212] relative flex items-center justify-center overflow-hidden shadow-[0_0_20px_rgba(0,0,0,0.8)] ring-2 ring-[#ccff00]/50 ring-offset-2 ring-offset-[#0a0a0a]">
                   {pAvatarUrl ? (
                     <img src={pAvatarUrl} alt={pName} className="object-cover" />
                   ) : (
                     <div className="bg-gradient-to-br from-[#ccff00]/10 to-transparent w-full h-full flex flex-col items-center justify-center p-2 pt-3 border border-[#ccff00]/20 rounded-full">
               ...
               <div className="flex items-center gap-2.5 mt-3">
                  {isAdmin ? (
                    <button 
                      ...
                      className="font-bold text-[14px] px-4 h-[36px] rounded-full border border-white/20 text-white hover:bg-white/10 transition-colors"
                    >
                      Edit profile
                    </button>
                  ) : (
            ...
            <div className="mt-2 text-left">
               <div className="flex items-center gap-1">
                  ...
                  <VerifiedBadge className="w-5 h-5 ml-1 drop-shadow-[0_0_8px_rgba(204,255,0,0.5)]" />
               </div>
                      ...
                      <Link href={`/post/${post.id}`} key={post.id + i} className="relative aspect-[3/4] bg-[#1a1a1a] rounded-lg overflow-hidden group border border-white/5">
                         ...
                         <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-2 pt-6 flex items-center justify-between">
                            <div className="flex items-center gap-1 text-white/80">
                              ...
                              <span className="text-[10px] font-medium">{post.viewsCount || 0}</span>
                            </div>
                            <div className="flex flex-row items-center gap-2">
                              <div className="flex items-center gap-1 text-white/80">
                                <Heart className="w-3 h-3" />
                                <span className="text-[10px] font-medium">{post.reactionsCount || 0}</span>
                              </div>
                              <div className="flex items-center gap-1 text-white/80">
